...
class CarDetailController {
    private static let errorInfoMessage = "There where no pictures for this car. Confirm your internet connection  and try again or maybe the car just don't have pictures."
    ...
    func searchPorschePictures(forPorscheId carId: String, completion: @escaping (Result<[Picture], DCError>) -> Void) {
        ...
        dataLoader.loadCarPicturesInformation(withCarId: carId) { [weak self] result in
            ...
            switch result {
            case let .success(carPicturesInformation):
...
private extension OSLog {
    static let carDetailController = OSLog.meliCarViewer("carDetailController")
}
